...
drop table specified_event_table;
...
create table specified_event_table(
    event_id            serial,
    event_league_id     int,
    ...
    venue_id            int,
    primary key (event_id, event_league_id)
);

alter table specified_event_table
...
create table all_event_table(
    general_event_id    serial          primary key,
    event_id            int             not null,
    event_league_id     int             not null
);

alter table all_event_table
    add constraint fk_ck_event_all_event foreign key (event_id, event_league_id)
        references specified_event_table (event_id, event_league_id);

create table performer_event_relation(
    performer_id        int             not null,
    general_event_id    int             not null,
    primary key (performer_id, general_event_id)
);
...
alter table performer_event_relation
    add constraint many_to_many_relation_to_all_event foreign key (general_event_id)
        references all_event_table (general_event_id);
...
alter table ticket_table
    add constraint fk_event_ticket foreign key (general_event_id)
        references all_event_table
     (general_event_id);
...
create table ticket_price(
    general_event_id        int,
    ticket_type     varchar(255),
    ticket_price    decimal(10, 2),
    primary key (general_event_id, ticket_type)
);

alter table ticket_table
    add constraint fk_ck_price_ticket_table foreign key (general_event_id, ticket_type)
        references ticket_price (general_event_id, ticket_type);
...
delete from all_event_table;
delete from specified_event_table;
...
\copy specified_event_table from 'C:/Users/rahar/Documents/Assignments/Tutorial/Node/Ticket/database/sample_data/specified_event_table.csv' delimiter ',' csv header;
\copy all_event_table from 'C:/Users/rahar/Documents/Assignments/Tutorial/Node/Ticket/database/sample_data/all_events.csv' delimiter ',' csv header;
...
left join specified_event_table se
